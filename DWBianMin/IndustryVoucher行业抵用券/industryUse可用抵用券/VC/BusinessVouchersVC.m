...
@interface BusinessVouchersVC ()<UITableViewDelegate,UITableViewDataSource>
...
@end
@implementation BusinessVouchersVC
...
-(void)setUpTableView{
    ...
    [_tableView tableViewregisterNibArray:@[@"IndustryListTwoCell"]];
}
#pragma mark - 关于数据
-(void)SET_DATA{
...
-(void)requestAction{
    ...
    NSMutableDictionary *dic  =[ @{@"pageIndex":@(self.pageIndex),@"pageCount":@(10),@"merchantId":self.industryModel.merchantId,@"amount":self.industryModel.amount}mutableCopy];
    __weak typeof(self) weakself = self;
    if (Token.length!= 0) {
        ...
        [[DWHelper shareHelper] requestDataWithParm:[baseReq yy_modelToJSONString] act:@"act=Api/Coupon/requestSelectCouponList" sign:[baseReq.data MD5Hash] requestMethod:GET success:^(id response) {
            NSLog(@"%@",response);
            ...
            if (baseRes.resultCode ==1) {
                ...
                for (NSDictionary *dicData in arr) {
                    ...
                    model.selected = [weakself.industryModel.couponId isEqualToString:model.couponId] ? YES : NO;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section>self.dataArray.count-1||self.dataArray.count==0) {
        ...
        IndustryListTwoCell * cell = [tableView dequeueReusableCellWithIdentifier:@"IndustryListTwoCell" forIndexPath:indexPath];
       // cell 赋值
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.selectStatus isEqualToString:@"1"]) {
        ...
        self.industryModel.couponId = model.couponId;
        for (IndustryModel*  model in self.dataArray) {
            model.selected =  [self.industryModel.couponId isEqualToString:model.couponId] ? YES : NO;
        }
         self.industryModel.couponType = model.couponType;
        ///1-满减券  2-立减券  3-折扣券
        if (model.couponType == 1) {
            self.industryModel.mVaule = model.mVaule;
        }else if (model.couponType == 2)  {
           
             self.industryModel.lValue =model.lValue;
        }else  if(model.couponType ==3 ){
            self.industryModel.dValue = model.dValue;
    };
        self.industryModel. couponUserId = model.couponUserId;
...
- (IBAction)ChoseBtnAction:(UIButton *)sender {
    sender.selected = YES;
    self.industryModel.couponId = @"";
     self.industryModel.couponType = 0;
    self.industryModel. couponUserId = @"";
    for (IndustryModel*  model in self.dataArray) {
        model.selected =  NO;
    }
    self.BusinessVouchersVCBlock(self.industryModel);
