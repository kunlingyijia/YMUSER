...
- (void)monthShow:(UIView *)myV {
    ...
    UILabel *monthLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 5, selfWidth, 20)];
    monthLabel.text = [self returnMonth:dateStr];
    monthLabel.textAlignment = NSTextAlignmentCenter;
    monthLabel.textColor = [UIColor colorWithHexString:@"#ff9712"];
    [myV addSubview:monthLabel];
    
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 26, selfWidth, 1)];
    line.backgroundColor = [UIColor colorWithHexString:@"#ff9712"];
    [myV addSubview:line];
    
}

- (NSString *)returnMonth:(NSString *)str {
...
- (void)createCalendarViewWith:(NSDate *)date {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        week.frame = CGRectMake((daysWidth) * i, 28, daysWidth, 20);
        week.backgroundColor = [UIColor clearColor];
        week.textAlignment = NSTextAlignmentCenter;
        week.textColor = [UIColor colorWithHexString:kSubTitleColor];
    ...
    for(int i = 0; i < 35; i ++) {
        ...
        CGFloat x = (i % 7) * (daysWidth-1)+3;
        CGFloat y = (i / 7) * (daysHeight-1) + daysHeight;
        
        // create Container view(button)
        UIButton *view = [[UIButton alloc] initWithFrame:CGRectMake(x, y, daysWidth, daysHeight)];
        view.layer.borderColor = [UIColor colorWithHexString:kViewBg].CGColor;
...
#warning the date include last month、this month、next month
        ...
        chineseCalendar.textColor = [UIColor colorWithHexString:kSubTitleColor];
//        chineseCalendar.superview.backgroundColor = [UIColor colorWithHexString:@"#ececec"];
...
-(void)buttonLongPress:(UILongPressGestureRecognizer *)gestureRecognizer{
    ...
    if ([gestureRecognizer state] == UIGestureRecognizerStateBegan) {
        ...
        popView *pop = [[popView alloc] initWithFrame:CGRectMake(0, 0, kWindowWidth, kWindowHeight)];
        [pop show];
    }
}
...
- (void)setStyle_BeyondThisMonth:(UIButton *)btn
{
    ...
    btn.superview.backgroundColor = [UIColor colorWithHexString:@"#ececec"];
}
...
- (void)setStyle_BeforeToday:(UIButton *)btn andIsSign:(BOOL )sign
{
    ...
    if(sign){
        ...
        UIImageView *signImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_my_qiandao_tubiao"]];
        [btn.superview addSubview:signImage];
        [signImage mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(btn.superview.mas_top).with.offset(5);
            make.right.equalTo(btn.superview.mas_right).with.offset(-5);
...
- (void)setStyle_Today:(UIButton *)btn andIsSign:(BOOL )sign
{
    ...
    if(sign){
        ...
        [btn.superview addSubview:signImage];
        [signImage mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(btn.superview.mas_top).with.offset(3);
            make.right.equalTo(btn.superview.mas_right).with.offset(-3);
            make.size.mas_equalTo(CGSizeMake(10, 10));
        }];
    }
    else{
...
- (void)setStyle_AfterToday:(UIButton *)btn
{
    btn.enabled = NO;
    [btn setTitleColor:[UIColor colorWithHexString:kTitleColor] forState:UIControlStateNormal];
    btn.superview.backgroundColor = [UIColor clearColor];
}
