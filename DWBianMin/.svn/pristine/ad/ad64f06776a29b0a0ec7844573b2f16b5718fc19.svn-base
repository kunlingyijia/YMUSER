...
- (void)addView {
    ...
    self.sellNumber.textColor = [UIColor colorWithHexString:kSubTitleColor];
    ...
    [self.name mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_equalTo(self.range.mas_left).with.offset(-SpaceOne);
...
//    }];
//
   // self.sellNumber.backgroundColor = [UIColor redColor];
    [self.sellNumber mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.right.equalTo(self.contentView).with.offset(-10);
    }];
...
- (void)cellGetDataModel:(RequestMerchantListModel *)model WithController:(BaseViewController *)controller{
    self.range.text = model.distance;
...
////        make.width.equalTo(@(80));
//
//        make.right.mas_equalTo(self.re).with.offset(100);
//        make.height.equalTo(@(nameWidth));
//    }];
    
    self.name.text = model.merchantName;
    //self.starRateView.scorePercent = model.star*0.2;
    self. sellNumber.text =[NSString stringWithFormat:@"销量:%@",model.sales ];
...
//    NSLog(@"%@", model.haveoffice);
    ...
    if (model.merchantType ==1) {
        ...
        if (subStar<0.25) {
            self.starRateView.scorePercent = star*0.2;

        }else if(subStar>0.75){
            self.starRateView.scorePercent = (star+1)*0.2;
        }else{
            self.starRateView.scorePercent = (star+0.5)*0.2;
