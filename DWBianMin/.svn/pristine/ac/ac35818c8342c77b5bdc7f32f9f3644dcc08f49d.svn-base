...
- (void)viewGetData {
    ...
    [self.confirmPayBtn setTitle:[NSString stringWithFormat:@"确认支付 ￥%.2f元",self.orderModel.payAmount] forState:(UIControlStateNormal)] ;
//    self.remark.text = self.orderModel.content;
...
- (IBAction)ConfirmPayBtnClick:(id)sender {
    ...
    NSLog(@"%@",[NSString stringWithFormat:@"%f" ,self.orderModel.payAmount ]);
    
    if (self.payType == 1) {
        
        [self getdataWith:[NSString stringWithFormat:@"%.2f" ,self.orderModel.payAmount ] withPayType:1];
...
//        }];
            ...
            [self getdataWith:[NSString stringWithFormat:@"%.2f" ,self.orderModel.payAmount ] withPayType:2];
//        UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"提示" message:@"请输入支付金额" preferredStyle:UIAlertControllerStyleAlert];
//        [alertC addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
