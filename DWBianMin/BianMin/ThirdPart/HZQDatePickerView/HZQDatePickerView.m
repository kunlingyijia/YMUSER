...
+ (HZQDatePickerView *)instanceDatePickerView:(NSString *)showTime
{
    NSArray* nibView =  [[NSBundle mainBundle] loadNibNamed:@"HZQDatePickerView" owner:nil options:nil];
    HZQDatePickerView *DDD = [nibView objectAtIndex:0];
    DDD.showTime = showTime;
    if ([showTime isEqualToString:@"1"]) {
        DDD.datePickerView.datePickerMode = UIDatePickerModeDateAndTime;
    }
...
- (NSString *)timeFormat
{
    ...
    if ([self.showTime isEqualToString:@"1"]) {
