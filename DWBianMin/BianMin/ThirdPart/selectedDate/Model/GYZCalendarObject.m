 ...
 *  日期模型
 */
@property (nonatomic, strong) GYZCalendarModel *model;
...
@implementation GYZCalendarObject
...
-(NSMutableArray *)reloadCalendarView:(NSDate *)date selectDate:(NSDate *)selectDate needDays:(int)days showType:(CalendarShowType)type isEnable:(BOOL)isEnable isChineseCalendar:(BOOL)isChineseCalendar {
    ...
    return [self reloadCalendarView:date selectDate:selectDate needDays:days showType:type isEnable:isEnable];
}
...
- (void)calculateDaysInPreviousMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    for (int i = daysCount - partialDaysCount + 1; i < daysCount + 1; ++i) {
        ...
        GYZCalendarModel *calendarDay = [GYZCalendarModel calendarWithYear:components.year month:components.month day:i];
...
- (void)changStyle:(GYZCalendarModel *)model
{
...
-(void)LunarForSolarYear:(GYZCalendarModel *)calendarDay{
