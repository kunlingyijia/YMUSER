...
- (void)LQPhotoPicker_initPickerView{
    ...
    //添加图片提示
    addImgStrLabel = [[UILabel alloc]initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width-64) /3+10, 50, 70, 20)];
...
- (void)changeCollectionViewHeight{
    ...
    if (_collectionFrameY) {
        ...
        _pickerCollectionView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-64.0) /3.0 +20.0)* ((int)(_LQPhotoPicker_selectedAssetArray.count)/3 +1)+20.0);
    }
...
//    UIImage *newImage = [self imageWithImageSimloe:img scaledToSize:CGSizeMake(320, 320)];
    
    return UIImageJPEGRepresentation(img, 0.5);
}

- (UIImage *)imageWithImageSimloe:(UIImage *)image scaledToSize:(CGSize)newSize {
